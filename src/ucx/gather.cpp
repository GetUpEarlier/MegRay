...
Status UcxCommunicator::gather(const void* sendbuff, void* recvbuff, size_t sendlen,
    ...
    if (m_rank == root) {
        for (size_t r = 0; r < m_nranks; r++) {
            ...
            if (r == root) {
                CUDA_CHECK(cudaMemcpy(p, sendbuff, bytes, cudaMemcpyDeviceToDevice));
            } else {
                MEGRAY_CHECK(_recv(p, bytes, r));
            }
        }
    } else {
        MEGRAY_CHECK(_send(sendbuff, bytes, root));
    }
    MEGRAY_CHECK(_flush());
    return MEGRAY_OK;
}
